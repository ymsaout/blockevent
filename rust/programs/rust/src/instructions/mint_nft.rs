...
pub fn mint_nft(ctx: Context<MintNft>) -> Result<()> {
    ...
    let init_token_meta_data_ix = &spl_token_metadata_interface::instruction::initialize(
        ...
        ctx.accounts.mint.key,
        ctx.accounts.nft_authority.to_account_info().key,
        "Vieilles Charrues en Rust".to_string(),
        "VCR".to_string(),
        "https://arweave.net/bPd0YFzZXiH6SgXuAqLIpV0vnPbn0PA8rJ1169nnZ8M".to_string()
    );
    ...
    // Mint one token to the associated token account of the owner
    token_2022::mint_to(
